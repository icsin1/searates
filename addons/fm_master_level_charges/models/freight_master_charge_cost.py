...
class MasterShipmentChargeCost(models.Model):
    ...
    def detach_charge_from_house_shipment(self):
        self.ensure_one()
        house_charge_billed = self.house_charge_cost_ids.filtered(
        ...
        if house_charge_billed:
            ...
            self.house_charge_cost_ids.unlink()
    ...
    def update_attached_charge_to_house_shipment(self, values):
        ...
        house_charge_billed = self.house_charge_cost_ids.filtered(
            lambda charge: charge.status in charge._modification_line_restrict_states())
        if house_charge_billed and not self.env.context.get('from_copy_method'):
            houses = house_charge_billed.mapped('house_shipment_id.name')
            raise ValidationError(
            ...
            if house_charge_values:
                self.house_charge_cost_ids.write(house_charge_values)
    ...
    def copy(self, default=None):
        ...
        record.house_shipment_id = False
        return record
