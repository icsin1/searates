...
class CustomerWiseReport(models.Model):
    _name = 'customer.wise.shipment.report'
    _description = 'Customer Wise Shipment Report'

    from_date = fields.Date(string='From Date')
    to_date = fields.Date(string='To Date')
    customer_ids = fields.Many2many('res.partner', string='Customers', domain=lambda self: [('category_ids', '=', self.env.ref('freight_base.org_type_customer').id)])
    company_id = fields.Many2one('res.company', string='Company', default=lambda self: self.env.company)

    @api.constrains('from_date', 'to_date')
    def from_to_date_constrains(self):
        for rec in self:
            if rec.from_date > rec.to_date:
                raise ValidationError(_('To date (%s) should be greater than From date (%s)', format_date(self.to_date), format_date(self.from_date)))
    ...
    def print_xlsx_report(self):
        ...
        worksheet.write(0, 7, 'CUSTOMER', style_1)
        ...
        for shipment in house_shipments:
            ...
            shipment_date = shipment.shipment_date and shipment.shipment_date.strftime(date_format) or ''
            cut_off_date = shipment.carrier_vessel_cut_off_datetime and shipment.carrier_vessel_cut_off_datetime.strftime(
                date_format) or ''
            ...
            for invoice in invoices:
                ...
                if invoice.invoice_date:
                    invoice_date = invoice.invoice_date.strftime(date_format)
                    invoice_date_list.append(invoice_date)
                if invoice.invoice_date_due:
                    invoice_date_due = invoice.invoice_date_due.strftime(date_format)
                ...
                if invoice.amount_total:
                    if invoice.currency_id == usd_currency:
                        ...
                        invoice_total_aed = invoice.currency_id._convert(invoice.amount_total, aed_currency,
                                                                         self.company_id,
                        ...
                        invoice_total_usd = invoice.currency_id._convert(invoice.amount_total, usd_currency,
                                                                         self.company_id,
                                                                         fields.Date.today()) if invoice.amount_total else 0
                    else:
                        invoice_total_usd = invoice.currency_id._convert(invoice.amount_total, usd_currency,
            ...
            shipper = shipment.client_id and shipment.client_id.name or ''
            ...
            if shipment.transport_mode_id.id == self.env.ref('freight_base.transport_mode_land').id:
                etd = shipment.etd_time and shipment.etd_time.strftime(date_format) or ''
                eta = shipment.etp_time and shipment.etp_time.strftime(date_format) or ''
                if shipment.etd_time and shipment.etp_time:
                    tt_days = shipment.etd_time.date() - shipment.etp_time.date()
                    transit_time = tt_days.days
            else:
                etd = shipment.etd_time and shipment.etd_time.strftime(date_format) or ''
                eta = shipment.eta_time and shipment.eta_time.strftime(date_format) or ''
        ...
        workbook.close()
        filename = 'Customer Wise Shipment Report %s-%s' % (self.from_date.strftime('%d%b%Y'), self.to_date.strftime('%d%b%Y'))
