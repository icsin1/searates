...
class ServiceJobChargeProFormaInvoiceWizard(models.TransientModel):
    _name = 'service.job.charge.pro.forma.invoice.wizard'
    ...
    charge_ids = fields.Many2many('service.job.charge.revenue', 'service_job_charge_pro_forma_invoice_wizard_charge_rel', 'wizard_id', 'charge_id', string='Charges',
                                  domain="[('service_job_id', '=', service_job_id), ('status', 'not in', ['pro_forma', 'partial', 'fully_invoiced'])]")
    line_ids = fields.One2many('service.job.charge.pro.forma.invoice.wizard.line', 'wizard_id', string='Invoice Lines')
    ...
    def _check_charge_billing_currency(self):
        for rec in self.filtered(lambda r: r.currency_id):
            ...
            service_job_currency = rec.service_job_id.currency_id
            is_service_job_currency_charge = all(c.amount_currency_id.id == service_job_currency.id for c in rec.charge_ids)
            if service_job_currency.name in charges_currency_names:
                charges_currency_names.remove(service_job_currency.name)
            charge_service_job_currency_names = charges_currency_names + [service_job_currency.name]
            if rec.single_currency_billing and not is_service_job_currency_charge:
                # rec.currency_id.id != shipment_currency.id:
                if len(charges_currency_names) > 1 and rec.currency_id.id != service_job_currency.id:
                    raise ValidationError(_('For different currency-charges Invoice-currency must be %s to generate Invoice.') % (service_job_currency.name))
                if len(charges_currency_names) == 1 and rec.currency_id.name not in charge_service_job_currency_names:
                    raise ValidationError(_('Invoice Currency can be %s only.') % (', '.join(charge_service_job_currency_names)))
            ...
            for currency in currencies:
                ...
                for charge in currency_charges:
                    ...
                    converted_amount = charge.amount_currency_id.with_context(currency_exchange_rate=currency_exchange_rate)._convert(
                        charge.total_currency_amount, currency, charge.service_job_id.company_id, charge.service_job_id.date
                    )
    ...
    def action_generate_pro_forma_invoices(self):
        ...
        return self.env['service.job.charge.revenue'].view_pro_forma_invoice(pro_forma_invoice)
...
class ServiceJobChargeProFormaInvoiceWizardLine(models.TransientModel):
    _name = 'service.job.charge.pro.forma.invoice.wizard.line'
    _inherit = 'journal.entry.generate.wizard.line.mixin'
    _description = 'Generate Pro-Forma Lines'

    wizard_id = fields.Many2one('service.job.charge.pro.forma.invoice.wizard', required=True, ondelete='cascade')
    partner_id = fields.Many2one('res.partner', string='Customer')
    currency_id = fields.Many2one('res.currency', string="Currency")
    charge_ids = fields.Many2many('service.job.charge.revenue', 'service_job_charge_pro_forma_invoice_line_wizard_charge_rel', 'wizard_id', 'charge_id', string='Charges')
    ...
    def _prepare_pro_forma_invoice(self):
        ...
        pro_forma_invoice_vals = {
            ...
            'charge_service_job_ids': [(6, 0, self.mapped('charge_ids').mapped(
                'service_job_id').ids or self.wizard_id.service_job_id.ids)],  # fro
    ...
    def _prepare_pro_forma_invoice_line(self):
        ...
        for charge in self.charge_ids:
            ...
            charge_amount = charge.amount_currency_id.with_context(
                currency_exchange_rate=currency_exchange_rate)._convert(charge.amount_currency_residual, self.currency_id, charge.service_job_id.company_id, charge.service_job_id.date)
            ...
            charge_rate = charge.amount_currency_id.with_context(
                currency_exchange_rate=currency_exchange_rate)._convert(charge.amount_rate, self.currency_id, charge.service_job_id.company_id, charge.service_job_id.date)
