...
class ServiceJobChargeRevenue(models.Model):
    ...
    pro_forma_invoice_line_ids = fields.One2many('pro.forma.invoice.line', 'service_job_charge_revenue_id', string='Pro Forma Invoices')
    ...
    def action_create_pro_forma_invoice(self):
        to_pro_forma_invoice = self.filtered(lambda inv: inv.status in ['no', 'pro_forma_cancel'])
        if not to_pro_forma_invoice:
            raise UserError(_("Nothing to pro-forma invoice."))
        if to_pro_forma_invoice[0].service_job_id.state == 'cancelled':
            raise UserError(_("Can not generate pro-forma invoice of cancelled service job."))

        self.check_charges_rate_per_unit('pro-forma invoice')
        action = self.env.ref('fm_service_job_proforma.service_job_charge_pro_forma_invoice_wizard_action').sudo().read([])[0]
        ...
        action['context'] = {
            ...
            'default_service_job_id': to_pro_forma_invoice[0].service_job_id.id,
    ...
    def _prepare_pro_forma_invoice(self):
        ...
        pro_forma_invoice_vals = {
            ...
            'company_id': self.service_job_id.company_id.id,
            'service_job_id': self.service_job_id.id,
    ...
    def _prepare_pro_forma_invoice_line(self):
        pro_forma_invoice_line_vals = {
            ...
            'service_job_charge_revenue_id': self.id
            }
    ...
    def view_pro_forma_invoice(self, pro_forma_invoice_ids):
        tree_view_id = self.env.ref('fm_service_job_proforma.proforma_invoice_service_job_view_tree').id
        ...
        if len(pro_forma_invoice_ids) > 1:
            service_job_ids = pro_forma_invoice_ids.mapped('service_job_id')
            action['domain'] = [('service_job_id', 'in', service_job_ids.ids)]
