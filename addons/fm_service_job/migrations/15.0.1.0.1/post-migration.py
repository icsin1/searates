...
def migrate(cr, version):
    ...
    cr.execute(""" SELECT id,customs_be_type FROM freight_service_job WHERE customs_be_type IS NOT NULL""")
    result = cr.fetchall()
    for rec in result:
        query = "UPDATE freight_service_job SET customs_be_type_id=%s WHERE id=%s" % (customs_be_type_dict[rec[1]], rec[0])
        cr.execute(query)
