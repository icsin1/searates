...
class AccountReconciliation(models.AbstractModel):
    ...
    def _domain_move_lines_for_reconciliation(
        ...
        aml_accounts = self.get_reconcile_accounts(st_line)
        ...
        domain_reconciliation = expression.AND([domain_reconciliation, [("balance", "!=", 0.0), ("parent_state", "=", "posted")]])
        domain = expression.OR([domain_reconciliation, domain_matching])
        if partner_id:
            domain = expression.AND([domain, [("partner_id", "=", partner_id)]])
        # NOTE
        # The following code has been commented due to the requirement to show all
        # invoices/bills on the Customer/Vendor Matching tab and journal entries on the Miscellaneous Matching tab.
        # The domain has been modified in the ics_account_reconciliation_widget_extended module.
        # if mode == "rp":
    ...
    def get_reconcile_accounts(self, st_line):
        return self.env['account.account'].search(
            [('reconcile', '=', True), ('company_id', '=', st_line.company_id.id)]).ids
