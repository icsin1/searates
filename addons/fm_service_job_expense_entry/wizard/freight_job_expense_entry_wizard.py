...
class FreightJobExpenseEntryWizard(models.TransientModel):
    _name = 'freight.job.expense.entry.wizard'
    _inherit = 'job.charge.bill.wizard'
    _description = 'Generating Expense Entry'

    company_id = fields.Many2one('res.company')
    charge_ids = fields.Many2many(
        'service.job.charge.cost', 'service_charge_cost_wizard_expense_rel', 'wizard_id', 'charge_id', string='Charges',
        domain="[('service_job_id', '=', service_job_id), ('status', '!=', 'fully_billed')]")

    line_ids = fields.One2many('freight.job.expense.entry.wizard.line', 'wizard_id', string='Bill Lines')
...
class FreightJobExpenseEntryWizardLine(models.TransientModel):
    _name = 'freight.job.expense.entry.wizard.line'
    _inherit = 'job.charge.bill.wizard.line'
    _description = 'Invoice Lines'

    wizard_id = fields.Many2one('freight.job.expense.entry.wizard', required=True, ondelete='cascade')
    charge_ids = fields.Many2many('service.job.charge.cost', 'service_charge_exp_entry_line_wizard_charge_rel', 'wizard_id', 'charge_id', string='Charges')
    ...
    def _generate_invoice(self, move_type):
        ...
        return {
            ...
            'ref': ','.join(self.wizard_id.service_job_id.mapped('name')),
