...
class AccountMove(models.Model):
    ...
    partner_credit = fields.Monetary(related='partner_id.commercial_partner_id.credit', readonly=True, currency_field='company_currency_id')
    partner_credit_limit = fields.Monetary(related='partner_id.credit_limit_compute', readonly=True, currency_field='company_currency_id')
    ...
    @api.depends('partner_credit_limit', 'partner_credit', 'invoice_line_ids', 'company_id.account_default_credit_limit', 'company_id.account_credit_limit')
    def _compute_show_partner_credit_warning(self):
        for move in self:
            ...
            partner_credit = move.partner_credit + move.amount_total_signed
            move.show_partner_credit_warning = account_credit_limit and (
                (company_limit and partner_credit > company_limit) or (partner_limit and partner_credit > partner_limit)
            )
