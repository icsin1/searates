...
class ShipmentExpenseEntryWizard(models.TransientModel):
    _name = 'shipment.expense.entry.wizard'
    _inherit = 'shipment.charge.bill.wizard'
    _description = 'Generating Expense Entry'

    company_id = fields.Many2one('res.company')
    charge_ids = fields.Many2many(
        'house.shipment.charge.cost', 'house_cost_wizard_expense_rel', 'wizard_id', 'charge_id', string='Charges',
        domain="[('house_shipment_id', '=', house_shipment_id), ('status', '!=', 'fully_billed')]")

    line_ids = fields.One2many('shipment.expense.entry.wizard.line', 'wizard_id', string='Bill Lines')
    journal_id = fields.Many2one('account.journal',
                                 domain='[("type", "=", "general"),("company_id", "=", company_id)]',
                                 required=True)
    payment_account_id = fields.Many2one('account.account',
                                         domain="[('user_type_id.internal_group', '=', 'asset'),('company_id','=', company_id)]",
                                         required=True)
...
class ShipmentExpenseEntryWizardLine(models.TransientModel):
    _name = 'shipment.expense.entry.wizard.line'
    _inherit = 'shipment.charge.bill.wizard.line'
    _description = 'Invoice Lines'

    wizard_id = fields.Many2one('shipment.expense.entry.wizard', required=True, ondelete='cascade')
    charge_ids = fields.Many2many('house.shipment.charge.cost', 'house_exp_entry_line_wizard_charge_rel', 'wizard_id',
                                  'charge_id', string='Charges')
    ...
    def _generate_invoice(self, move_type):
        ...
        return {
            ...
            'ref': ','.join(self.wizard_id.house_shipment_id.mapped('name')),
            'invoice_user_id': self.env.user.id,
            'journal_id': self.wizard_id.journal_id.id,
            'invoice_origin': house_shipment.display_name,
            ...
            'charge_house_shipment_ids': [(6, 0, self.wizard_id.house_shipment_id.ids)]
        }
    ...
    def _prepare_invoice_line(self):
        ...
        for charge in self.charge_ids:
            ...
            charge_rate = charge.amount_currency_id.with_context(
                ...
                charge.amount_rate,
                self.currency_id,
                charge.house_shipment_id.company_id,
                charge.house_shipment_id.shipment_date,
                round=False
            ) or 1
