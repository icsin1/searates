...
class HouseShipmentChargeCost(models.Model):
    ...
    def action_create_expense_entry(self):
        ...
        action = self.env.ref('fm_shipment_expense_entry.house_shipment_expense_entry_wizard_action').sudo().read([])[0]
        action['context'] = {
    ...
    def _get_action_for_view_moves(self):
        ...
        if 'entry' in self.move_line_ids.mapped('move_id.move_type'):
            ...
            action['context'] = {'default_house_shipment_id': self.id, 'default_move_type': 'entry', 'create': 0, 'search_default_posted': 1}
