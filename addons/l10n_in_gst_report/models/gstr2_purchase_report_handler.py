...
class GSTR2BPurchaseReportHandler(models.AbstractModel):
    _name = 'gstr2.purchase.report.handler'
    _description = 'GSTR2 Purchase Handler'
    ...
    def _get_sections(self, report, options, **kwargs):
        ...
        for report_group_key, report_group in report_groups.items():
            records = DataModel.search(report_group.get('__domain', []))
            for move in records:
                ...
                tax_amount_data_dict = self.get_account_move_tax_amount(move)
                ...
                move_values = self._generate_values(report, self._process_values(move, {
                    ...
                    'gst_treatment': self.get_invoice_type().get(gst_treatment, ''),
                    'document_type': self._get_voucher_type(move),
                    'document_number': move.ref,
                    'document_date': move.invoice_date.strftime('%d/%m/%Y'),
    ...
    def get_account_move_tax_amount(self, move_id):
    ...
    def get_invoice_type(self):
        return {
            'regular': 'B2B',
            'deemed_export': 'DE',
            'special_economic_zone_with_payment': 'SEZWP',
            'special_economic_zone_without_payment': 'SEZWOP',
        }
    ...
    def generate_csv_report(self, writer, data, record):
        ...
        DataModel = self.env[report.model_name]
        move_ids = DataModel.search(report._get_default_domain(options, **kwargs))
        for move in move_ids:
            gst_treatment = move.l10n_in_gst_treatment if move.l10n_in_gst_treatment != 'special_economic_zone' else \
                '{}_{}'.format(move.l10n_in_gst_treatment, move.l10n_in_export_type)
            tax_amount_data_dict = self.get_account_move_tax_amount(move)
            writer.writerow({
                'GSTIN of supplier': move.partner_id.vat or '',
                'Trade/Legal name': move.partner_id.name or '',
                'Type of inward supplies': self.get_invoice_type().get(gst_treatment, ''),
                'Document type': self._get_voucher_type(move),
                'Document number': move.ref or '',
                'Document date': move.invoice_date.strftime('%d/%m/%Y'),
                'Taxable value': abs(move.amount_untaxed_signed),
                'Integrated tax': tax_amount_data_dict['igst_amount'],
                'Central tax': tax_amount_data_dict['cgst_amount'],
                'State/ UT tax': tax_amount_data_dict['sgst_amount'],
                'Cess': tax_amount_data_dict['cess_amount'],
            })
