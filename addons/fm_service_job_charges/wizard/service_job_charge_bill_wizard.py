...
class ServiceJobChargeBillWizard(models.TransientModel):
    _name = 'job.charge.bill.wizard'
    ...
    def _compute_show_exchange_rate(self):
        for rec in self:
            is_service_job_charge = all(c.amount_currency_id.id == rec.currency_id.id for c in rec.charge_ids)
            rec.show_exchange_rate = True if rec.single_currency_billing and rec.service_job_id.currency_id.id != rec.currency_id.id and not is_service_job_charge else False
    ...
    charge_ids = fields.Many2many(
        'service.job.charge.cost', 'service_charge_cost_wizard_charge_rel', 'wizard_id', 'charge_id', string='Charges',
        domain="[('service_job_id', '=', service_job_id), ('status', '!=', 'fully_billed')]")
    line_ids = fields.One2many('job.charge.bill.wizard.line', 'wizard_id', string='Bill Lines')
            ...
            for currency in currencies:
                ...
                if total_amount:
                    lines.append((0, 0, {
                        ...
                        'charge_ids': [(6, False, partner_charges.ids)],
                        'amount': total_amount,
...
class ServiceJobChargeBillWizardLine(models.TransientModel):
    _name = 'job.charge.bill.wizard.line'
    _inherit = 'journal.entry.generate.wizard.line.mixin'
    _description = 'Invoice Lines'

    wizard_id = fields.Many2one('job.charge.bill.wizard', required=True, ondelete='cascade')
    partner_id = fields.Many2one('res.partner', string='Bill From')
    currency_id = fields.Many2one('res.currency', string="Bill Currency")
    charge_ids = fields.Many2many('service.job.charge.cost', 'service_charge_bill_line_wizard_charge_rel', 'wizard_id', 'charge_id', string='Charges')
    ...
    def _prepare_invoice_line(self, move_type):
        ...
        for charge in self.charge_ids:
            ...
            charge_amount = charge.amount_currency_id.with_context(currency_exchange_rate=currency_exchange_rate)._convert(
                charge.amount_currency_residual,
                self.currency_id,
                charge.service_job_id.company_id,
                charge.service_job_id.date,
                round=False
            )
            ...
            quantity = amount_for_charge / charge_rate if self.is_partial_invoice or charge.status == 'partial' else charge.quantity  # decreasing quantity for partial-bill as rate can be fixed
            ...
            charge_lines.append((0, 0, {
                ...
                'service_job_charge_cost_id': charge.id,
