...
class FreightMasterShipmentRoute(models.Model):
    _name = 'freight.master.shipment.route'
    _description = 'Master Shipment route'
    _inherit = ['freight.shipment.route.mixin']

    shipment_id = fields.Many2one('freight.master.shipment', required=True, ondelete='cascade')
    shipment_transport_mode_id = fields.Many2one(related='shipment_id.transport_mode_id', store=True, string='Shipment Transport')
