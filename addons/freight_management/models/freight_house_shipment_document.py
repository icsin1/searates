...
class FreightShipmentDocument(models.Model):
    _name = 'freight.house.shipment.document'
    _inherit = ['freight.document.mixin', 'document.validation.mixin']
    _description = 'Freight Shipment Document'

    shipment_id = fields.Many2one('freight.house.shipment', string='Shipment', required=True, ondelete='cascade')
    ...
    def action_doc_version_history(self):
        ...
        return {
            ...
            'domain': [('res_id', '=', self.id), ('res_model', '=', 'freight.house.shipment.document')]
        }
    ...
    def action_send_by_email(self):
        return self.generate_send_by_email_action(
            ...
            self.shipment_id.client_id.ids
        )
