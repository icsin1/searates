...
class ShipmentChargeInvoiceWizard(models.TransientModel):
    _name = 'shipment.charge.invoice.wizard'
    ...
    charge_ids = fields.Many2many(
        'house.shipment.charge.revenue', 'charge_invoice_wizard_charge_rel', 'wizard_id', 'charge_id', string='Charges',
        domain="[('house_shipment_id', '=', house_shipment_id), ('status', 'not in', ['pro_forma', 'pro_forma_cancel', 'fully_invoiced'])]")
    line_ids = fields.One2many('shipment.charge.invoice.wizard.line', 'wizard_id', string='Invoice Lines')
    ...
    def _onchange_currency_id(self):
        for rec in self.filtered(lambda r: r.currency_id):
            existing_charge = rec.charge_ids.filtered(lambda charge: charge.amount_currency_id == rec.currency_id)
            if existing_charge:
                rec.amount_conversion_rate = existing_charge[0].amount_conversion_rate
            else:
    ...
    def _check_partial_invoice_currency_rate(self):
        for rec in self:
            for charge in rec.charge_ids.filtered(lambda c: c.status == 'partial'):
                ...
                to_invoice_rate = rec.get_exchange_rate(charge)
...
class ShipmentChargeInvoiceWizardLine(models.TransientModel):
    _name = 'shipment.charge.invoice.wizard.line'
    _inherit = 'journal.entry.generate.wizard.line.mixin'
    _description = 'Invoice Lines'

    wizard_id = fields.Many2one('shipment.charge.invoice.wizard', required=True, ondelete='cascade')
    partner_id = fields.Many2one('res.partner', string='Invoice To')
    currency_id = fields.Many2one('res.currency', string="Invoice Currency")
    charge_ids = fields.Many2many('house.shipment.charge.revenue', 'charge_invoice_line_wizard_charge_rel', 'wizard_id', 'charge_id', string='Charges')
    ...
    def _generate_invoice(self, move_type):
        ...
        invoice_line_vals = self._prepare_invoice_line(move_type)
        attached_house_shipment_ids = [line[2].get('house_shipment_id') for line in invoice_line_vals]
        invoice_cash_rounding_id = self.wizard_id.invoice_cash_rounding_id
        return {
            'move_type': move_type,
            'currency_id': self.currency_id.id,
            'user_id': self.env.user.id,
            'invoice_user_id': house_shipment.sales_agent_id.id,
            ...
            'ref': house_shipment.booking_nomination_no,
            ...
            'charge_house_shipment_ids': [(6, 0, attached_house_shipment_ids)],
    ...
    def _prepare_invoice_line(self, move_type):
        ...
        for charge in self.charge_ids:
            ...
            amount_currency = charge.amount_currency_id.with_context(currency_exchange_rate=currency_exchange_rate)
            charge_amount = amount_currency._convert(charge.amount_currency_residual, self.currency_id, charge.house_shipment_id.company_id, charge.house_shipment_id.shipment_date, round=False)
            charge_ratio = charge_amount / (self.full_amount or 1)
            # Charge rate converted to requested currency
            charge_rate = amount_currency._convert(charge.amount_rate, self.currency_id, charge.house_shipment_id.company_id, charge.house_shipment_id.shipment_date, round=False) or 1
            ...
            charge_lines.append((0, 0, {
                ...
                'house_shipment_id': charge.house_shipment_id.id,
            }))
        return charge_lines
