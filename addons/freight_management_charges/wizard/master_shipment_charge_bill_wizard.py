...
class MasterShipmentChargeBillWizard(models.TransientModel):
    _name = 'master.shipment.charge.bill.wizard'
    _inherit = 'journal.entry.generate.wizard.mixin'
    _description = 'Generating Bills'

    @api.depends('single_currency_billing', 'master_shipment_id', 'currency_id')
    def _compute_show_exchange_rate(self):
        for rec in self:
            ...
            rec.show_exchange_rate = True if rec.single_currency_billing and rec.master_shipment_id.currency_id.id != rec.currency_id.id and not is_shipment_currency_charge else False

    house_shipment_id = fields.Many2one('freight.house.shipment', ondelete='cascade')
    master_shipment_id = fields.Many2one('freight.master.shipment', ondelete='cascade')
    partner_mode = fields.Selection(string='Bills for Vendors')
    charge_ids = fields.Many2many(
        'master.shipment.charge.cost', 'master_charge_cost_wizard_charge_rel', 'wizard_id', 'charge_id', string='Charges',
        domain="[('master_shipment_id', '=', master_shipment_id), ('status', '=', 'no')]")
    line_ids = fields.One2many('master.shipment.charge.bill.wizard.line', 'wizard_id', string='Bill Lines')
    ...
    def _onchange_currency_id(self):
        for rec in self.filtered(lambda r: r.currency_id):
            rec.amount_conversion_rate = 1 / (self.currency_id._get_conversion_rate(
                rec.master_shipment_id.currency_id, rec.currency_id, rec.master_shipment_id.company_id, rec.master_shipment_id.shipment_date) or 1.0)

    def get_exchange_rate(self, charge):
        self.ensure_one()
        if self.single_currency_billing and self.currency_id.id != charge.amount_currency_id.id and charge.amount_currency_id.id == self.master_shipment_id.currency_id.id:
        ...
        for rec in self:
            for charge in rec.charge_ids.filtered(lambda c: c.status == 'partial'):
                ...
                to_invoice_rate = charge.amount_currency_id.with_context(currency_exchange_rate=rec.get_exchange_rate(charge))._get_conversion_rate(
                    charge.amount_currency_id, to_invoice_currency, self.master_shipment_id.company_id, self.master_shipment_id.shipment_date)
    ...
    def _check_charge_billing_currency(self):
        for rec in self.filtered(lambda r: r.currency_id):
            ...
            shipment_currency = rec.master_shipment_id.currency_id
            ...
            for currency in currencies:
                ...
                for charge in currency_charges:
                    ...
                    converted_amount = charge.amount_currency_id.with_context(currency_exchange_rate=currency_exchange_rate)._convert(
                        charge.amount_currency_residual, currency, self.master_shipment_id.company_id, self.master_shipment_id.shipment_date
                    )
...
class ShipmentChargeBillWizardLine(models.TransientModel):
    _name = 'master.shipment.charge.bill.wizard.line'
    _inherit = 'journal.entry.generate.wizard.line.mixin'
    _description = 'Invoice Lines'

    wizard_id = fields.Many2one('master.shipment.charge.bill.wizard', required=True, ondelete='cascade')
    partner_id = fields.Many2one('res.partner', string='Bill From')
    currency_id = fields.Many2one('res.currency', string="Bill Currency")
    charge_ids = fields.Many2many('master.shipment.charge.cost', 'master_charge_bill_line_wizard_charge_rel', 'wizard_id', 'charge_id', string='Charges')

    def _generate_invoice(self, move_type):
        self.ensure_one()
        master_shipment = self.wizard_id.master_shipment_id
        AccountMove = self.env['account.move'].with_context(default_move_type=move_type, default_company_id=master_shipment.company_id.id)  # Keep Shipment company to get journal
        ...
        return {
            ...
            'invoice_origin': master_shipment.name,
            'booking_reference': master_shipment.name,
    ...
    def _prepare_invoice_line(self, move_type):
        ...
        charge_lines = []
        for charge in self.charge_ids:
            charge_amount = charge.amount_currency_id._convert(charge.amount_currency_residual, self.currency_id, charge.master_shipment_id.company_id, charge.master_shipment_id.shipment_date)
            charge_ratio = charge_amount / (self.full_amount or 1)
            # Charge rate converted to requested currency
            charge_rate = charge.amount_currency_id._convert(charge.amount_rate, self.currency_id, charge.master_shipment_id.company_id, charge.master_shipment_id.shipment_date) or 1
